...
const settings = require('./shared/db') && null; // avoid native; use default shape inline
const def = {
  prefix:'!',
...
(async()=>{
  await ejs.renderFile(path.join(views,'index.ejs'), {user}, opts);
  console.log('index.ejs OK');
  await ejs.renderFile(path.join(views,'dashboard.ejs'), {user, guilds:[guild]}, opts);
  console.log('dashboard.ejs OK');
  await ejs.renderFile(path.join(views,'guild.ejs'), {user, guild, settings:def, saved:'moderation'}, opts);
  console.log('guild.ejs OK');
